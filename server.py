...
async def broadcast(payload, target=None):
    groups = [target] if target else ['display','admin']
    dead=[]
    for g in groups:
...
async def ws_handler(request):
    ...
    await ws.prepare(request); clients[role].add(ws)
        ...
        async for msg in ws:
            ...
            if role=='admin':
                if typ=='command':
                    ...
                    print(f"ADMIN COMMAND RECEIVED: {cmd}", flush=True)
                    if cmd=='reload': await broadcast({'type':'command','command':'reload'}, 'display')
                        ...
                        await ws.send_json({'type':'notice','message':'Reboot requested'})
                        asyncio.create_task(reboot_later())
            elif typ=='screen_state':
                await broadcast({'type':'screen_state','version':data.get('version',VERSION),'world':data.get('world')}, 'admin')
    finally:
